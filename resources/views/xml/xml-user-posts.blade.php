...
<?xml-stylesheet type="text/xsl" href="{{ asset('storage/xml/user-style-browse.xsl') }}"?>
...
<pins xmlns:media="http://search.yahoo.com/mrss/">
    ...
    <user>
        ...
        <profile>{{ url('/profile/' . $user->nickname) }}</profile>
        <motto>{{ $user->motto }}</motto>
        <image>{{ asset('storage/' . $user->image_folder) }}</image>
    </user>
